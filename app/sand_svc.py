...
class SandService(BaseService):
            ...
            if platform in compile_options.keys():
                ...
                await self._compile_new_agent(platform=platform,
                                              ...
                                              **compile_options[platform],
                                              flag_params=('server', 'c2', 'listenP2P', 'fetchPeers'),
                                              extension_names=extension_names)
    ...
    async def _compile_new_agent(self, platform, headers, compile_target_name, output_name, buildmode='',
                                 extldflags='', cflags='', flag_params=('server', 'c2', 'listenP2P', 'fetchPeers'),
        ...
        for param in flag_params:
            if param in headers:
                if param == 'c2':
                    ...
                    onlineHostnames = [agent.host for agent in await self.data_svc.locate('agents')]
                    if onlineHostnames:
                        ldflags.append('-X main.%s=%s' % ('onlineHosts', ','.join(onlineHostnames)))
                else:

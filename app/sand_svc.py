...
class SandService:
    ...
    async def registration(self, paw, platform, server, host, group):
        ...
        if agent:
            ...
            aa = dict(hostname=host, last_seen=datetime.now(), paw=paw, checks=1, platform=platform, sleep=60, server=server)
    ...
    async def check_in(self, paw):
        ...
        if not agent:
            ...
            return
        updated_host = dict(last_seen=datetime.now(), checks=agent[0]['checks'] + 1)
    ...
    async def instructions(self, agent):
        ...
        for link in await self.data_svc.dao.raw_select(sql):
            ...
            instructions['commands'].append(json.dumps(dict(id=link['id'], sleep=link['jitter'], command=link['command'])))
    ...
    async def post_results(self, paw, link_id, output, status):
        ...
        await self.data_svc.dao.update('core_chain', key='id', value=link_id, data=dict(status=int(status), finish=datetime.now()))
